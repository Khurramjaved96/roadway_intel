...
def send_msg(sock, msg):
    ...
    :param sock: Socket to which data has to be sent. 
    :param msg: Msg that has to be sent. 
    """
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            #Read and resize the image to detect text lines.
            im = cv2.imread(os.path.join('/CTPN/ctpn_server/', filename))
            im, f=resize_im(im, cfg.SCALE, cfg.MAX_SCALE)

            #Detect lines using text detector.
            text_lines=myDetector.text_detector.detect(im)
            print "Number of the detected text lines: %s"%len(text_lines)


            # Establish the connection with Recognition server
            ...
            for idx, box in enumerate(text_lines):
                if utils.reject(labels[idx], falseRecognitions):
                    continue
